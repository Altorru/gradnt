...
export function HomeScreen() {
  ...
  const isGuidedStart = activitiesQuery.isSuccess && activities.length === 0
          ...
          */}
          ...
          {isGuidedStart ? (
            <GradntCard accent gap="$4">
              <YStack gap="$2">
                <GradntHeading level={3}>{t('home.firstStep.title')}</GradntHeading>
                <GradntText>{t('home.firstStep.description')}</GradntText>
              </YStack>
              {showStravaConnect ? (
                <GradntStravaConnectBlock
                  ...
                  title={t('home.firstStep.connectTitle')}
                  description={t('home.firstStep.connectDescription')}
                  connectLabel={t('settings.strava.connect')}
                  connectingLabel={t('settings.strava.connecting')}
                />
              ) : (
                <GradntText muted>{t('home.firstStep.syncing')}</GradntText>
              )}
              <GradntButton tone="ghost" onPress={() => router.push('/rides/new')}>
                {t('home.firstStep.addRide')}
              </GradntButton>
            </GradntCard>
          ) : null}

          <YStack display={isGuidedStart ? 'none' : 'flex'} gap="$5">
